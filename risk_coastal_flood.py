...
def run(lat, lon):
    ...
    # Define a small buffer around the point (in meters)
    buffer = 76 / 2
    minx = x - buffer
    maxx = x + buffer
    miny = y - buffer
    ...
    data100 = requests.get(url100).json()
    data500 = requests.get(url500).json()

    output = {}

    if data100.get("features"):
